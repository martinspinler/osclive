...
class MidiAdapter(AbstractAdapter):
    ...
    def _read(self, timeout: int = 2) -> list[int]:
...
class SLRaw1394Backend(SLBackend):
    def __init__(self, device: RawStudioLiveDevice, midi: Optional[str] = None) -> None:
        super().__init__()
    ...
    def _update_channel(self, ch: RawChannel, data: list[int]) -> None:
        ...
        for control, pos in ch.info.ctrls.items():
            ...
            if self._update_control(ch, control, value):
                handled = True
                print("control", control, data[pos.byte:pos.byte+2])
